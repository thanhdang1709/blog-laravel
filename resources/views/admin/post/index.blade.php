    ...
    <div class="col-12">
        <div class="card-box">
            <div class="btn-header mb-1">
                <button onclick="window.location.href='{{route('post.create')}}'" class="btn btn-success"><span class="fas fa-plus"></span> Add post</button>
...
<table     data-toggle="table"
        ...
        <tbody>
            @foreach ($posts as $post)
                <tr>
                    <td><a href="{{route('post.edit',$post->id)}}"><span class="fas fa-edit"></span></a>  <a href="{{route('admin.post.delete',$post->id)}}"><span class="fas fa-trash text-danger"></span></a></td>
                    <td>{{$post->title}}</td>
                    <td>{{$post->slug}}</td>
                    <td><span class="badge badge-success">{{$post->status}}</span></td>
                    <td><span class="badge badge-{{$post->comments==1?'success':'warning'}}">{{$post->comments==1?'active':'disable'}}</span></td>
                    <td><span class="badge badge-{{$post->featured==1?'success':'warning'}}">{{$post->comments==1?'active':'disable'}}</span></td>
    ...
    <div class="row text-center">
                 {{ $posts->links('admin.vendor.pagination.bootstrap-4')}}
    </div>
        </div>
    </div>
</div>
